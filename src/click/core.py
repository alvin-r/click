...
class Context:
    ...
    def __init__(
        ...
        self.obj: t.Any = obj or (parent.obj if parent else None)
        self._meta: dict[str, t.Any] = getattr(parent, "meta", {})

        self.default_map: cabc.MutableMapping[str, t.Any] | None = (
            default_map if default_map is not None else (
                parent.default_map.get(info_name) if info_name and parent and parent.default_map else None
            )
        )

        self.invoked_subcommand: str | None = None

        self.terminal_width: int | None = terminal_width or (parent.terminal_width if parent else None)
        self.max_content_width: int | None = max_content_width or (parent.max_content_width if parent else None)
        ...
        self.help_option_names: list[str] = help_option_names or (parent.help_option_names if parent else ["--help"])
        self.token_normalize_func: t.Callable[[str], str] | None = token_normalize_func or (parent.token_normalize_func if parent else None)
        
        self.resilient_parsing: bool = resilient_parsing
        
        self.auto_envvar_prefix: str | None = (
            auto_envvar_prefix.upper().replace("-", "_") 
            if auto_envvar_prefix is not None else (
                f"{parent.auto_envvar_prefix}_{self.info_name.upper()}".replace("-", "_")
                if parent and parent.auto_envvar_prefix and self.info_name 
                else None
            )
        )
