...
class Context:
    ...
    def __init__(
        ...
        self.default_map = (
            parent.default_map.get(info_name) if (default_map is None and parent and parent.default_map) else default_map
        )

        self.invoked_subcommand: str | None = None

        self.terminal_width = terminal_width or (parent.terminal_width if parent else None)

        self.max_content_width = max_content_width or (parent.max_content_width if parent else None)
        ...
        self.help_option_names = help_option_names if help_option_names is not None else (parent.help_option_names if parent else ["--help"])

        self.token_normalize_func = token_normalize_func or (parent.token_normalize_func if parent else None)
        ...
        elif auto_envvar_prefix:
            auto_envvar_prefix = auto_envvar_prefix.upper().replace("-", "_")
        self.auto_envvar_prefix = auto_envvar_prefix

        self.color = color if color is not None else (parent.color if parent else None)

        self.show_default = show_default if show_default is not None else (parent.show_default if parent else None)
